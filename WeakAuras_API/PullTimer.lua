...
local function GetBreakTimerDuration(duration)
  return duration < 60 and duration * 60 or duration
end
...
local function GetPlayerFullName()
  ...
  return realm and ("%s-%s"):format(name, realm) or name
end

local function SendDBMBreakTimer(duration, channel)
  local dbmDuration = GetBreakTimerDuration(duration)
  local playerName = GetPlayerFullName()
  C_ChatInfo.SendAddonMessage("D5", playerName .. "\t1\tBT\t" .. dbmDuration, channel)
  C_ChatInfo.SendAddonMessage("BigWigs", "P^Break^" .. dbmDuration, channel)
end
...
local function HandleTimerCommand(msg, timerType)
  ...
  if timerType == "PULL" then
    ...
    SendDBMBreakTimer(duration, sendChannel)
    db.breakStartTime = time()
    db.breakExpTime = db.breakStartTime + GetBreakTimerDuration(duration)
  end
...
local function StartTimer(duration, senderName, timerType)
  ...
  if isLeader or isAssistant and not IsSolo() then
    if timerType == "BREAK" and duration > 0 then
      duration = GetBreakTimerDuration(duration)
    end
...
function MerfinPlus:HandleChatMsgAddon(event, prefix, msg, channel, sender)
  ...
  elseif prefix == "BigWigs" then
    local _, timerType, duration = strsplit("^", msg)
    duration = tonumber(duration)
    if type(timerType) ~= "string" or not duration then
      return
    end
    -- Get unit name of sender without server name
    local senderName = Ambiguate(sender, "none")
