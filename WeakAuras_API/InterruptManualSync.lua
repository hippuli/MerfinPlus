...
local function CanSend()
  if type(MerfinPlus.CanBroadcastInterruptAssignments) == "function" then
    return MerfinPlus:CanBroadcastInterruptAssignments()
  end
...
local function Normalize(snapshot, bossKey, sourceMode)
  if type(MerfinPlus.NormalizeInterruptAssignmentsSnapshot) ~= "function" then return nil end
  return MerfinPlus:NormalizeInterruptAssignmentsSnapshot(snapshot, sourceMode, bossKey)
end
...
function MerfinPlus:BroadcastInterruptAssignmentsSnapshot(snapshot, bossKey, sourceMode, trigger)
  local allowed, reason = CanSend()
  if not allowed then return false, reason end
  local normalized, normalizeError = Normalize(snapshot, bossKey, sourceMode)
  if not normalized then return false, normalizeError or "Interrupt assignments are invalid." end
  local payload = Encode(normalized)
  if not payload or #payload > MAX_BYTES then return false, "Interrupt assignments could not be encoded." end
  RegisterPrefix()
  local total = math.ceil(#payload / CHUNK_BYTES)
  if total < 1 or total > MAX_CHUNKS then return false, "Interrupt assignment sync is too large." end
  ...
  local transfer = tostring((GetServerTime and GetServerTime()) or (time and time()) or 0)
    .. "-" .. tostring(self.interruptManualSequence)
  outgoing[transfer] = {
    bossKey = normalized.bossKey,
    acknowledged = {},
    rejected = {},
  }
  ...
  for index = 1, total do
    ...
    if not Send(PREFIX, table.concat({ "D", transfer, index, total, chunk }, "|"), channel) then
      outgoing[transfer] = nil
      return false, "Interrupt assignment sync could not be submitted."
    end
  end
  self:PublishReceivedInterruptManualSnapshot(normalized, UnitName and UnitName("player"))
...
function MerfinPlus:BroadcastInterruptManualAssignments(snapshot)
  local bossKey = type(snapshot) == "table" and (snapshot.bossKey
    or (type(snapshot.encounter) == "table" and snapshot.encounter.key))
  return self:BroadcastInterruptAssignmentsSnapshot(snapshot, bossKey, "manual", "legacy-manual")
end
...
frame:SetScript("OnEvent", function(_, event, prefix, message, channel, sender)
  ...
  if snapshot and MerfinPlus:PublishReceivedInterruptManualSnapshot(snapshot, sender) then
