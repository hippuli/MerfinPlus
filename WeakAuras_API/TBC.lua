...
local channelMain = {
  ...
  ["PRIEST"] = C_Spell.GetSpellName(25387),
}
...
local IsSafeToCast = function(unitId, debuffs, isCast)
  ...
  for _, spellId in ipairs(debuffs) do
    ...
    debuffName, _, _, _, _, expTime = WA_GetUnitDebuff(unitId, spellName, "PLAYER|HARMFUL")
    if debuffName then
      break
    end
  end
  ...
  if isCasting then
    ...
    if isCasting == channelSpell then
      if myClass == "WARLOCK" then
        ...
        local totalChannelTime = (endTimeMS - startTimeMS) / 1000
        local tilNextTick = remCastTime % (totalChannelTime / 3)
        return (tilNextTick + castTime) > remDebuffTime
      end
    end
